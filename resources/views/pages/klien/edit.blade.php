...
@section('content')
<div class="mb-6">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Edit Data Klien</h2>
    <p class="text-sm text-gray-500 dark:text-gray-400">Ubah data klien yang diperlukan</p>
</div>
    ...
    <form method="POST" action="{{ route('klien.update', $klien->id) }}">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="nama_klien" value="{{ old('nama_klien', $klien->nama_klien) }}"
            ...
            <div>
                ...
                <input type="text" name="no_telpon" value="{{ old('no_telpon', $klien->no_telpon) }}"
                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
            </div>
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="Tidak Aktif" {{ old('status', $klien->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="tanggal_kerjasama" value="{{ old('tanggal_kerjasama', $klien->tanggal_kerjasama) }}"
                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" name="alamat" value="{{ old('alamat', $klien->alamat) }}"
                    class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="catatan" rows="3"
                    class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:outline-none focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">{{ old('catatan', $klien->catatan) }}</textarea>
            </div>
            {{-- Kategori --}}
            <div class="sm:col-span-2">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Kategori</label>
                <input type="text" name="kategori" value="{{ old('kategori', $klien->kategori) }}"
